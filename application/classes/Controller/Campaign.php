    ...
    public function action_new(){
        ...
        if ($posted_form) {
            ...
            if ($validation->check()) {
                ...
                if ($login_success) {
                    HTTP::redirect('wepayapi?demo=' . $posted_form['enable_demo']);
                } else{
   		...
   		if (Auth::instance()->logged_in()) {
			...
			if ($campaign->email == $user->email || $user->login_role == 'admin') {
				...
				if ($campaign->hasAccountId()) {
					$this->vars('wepay', "<a href=" . URL::base() . "user/create_credit_card/".$id." class='btn btn-danger btn-large' id='buy-now-button'>Donate to ".$campaign->campaign_name." Now!</a>");
					$this->vars('token', true);
				} else {
					$this->vars('wepay', '');
					$this->vars('token', false);
